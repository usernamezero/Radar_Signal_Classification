...
def bpf(signals, fs, fc1, fc2, numtaps=3, mode='iir'):
    if mode == 'iir':
        b, a = signal.iirfilter(numtaps, [fc1, fc2], fs=fs)
    elif mode == 'fir':
        b = signal.firwin(numtaps, [fc1, fc2], fs=fs, pass_zero=False)
        a = 1
    return signal.lfilter(b, a, signals)


def wave_filter(signal, wave='dbN', level=6, usedcoeffs=[1, 1, 0, 0, 0, 0, 0]):
    """
...
# 信号绘制时频图
def signal2spectrum(data, stft_window_size, stft_stride, cwt_wavename, cwt_scale_num, n_downsample=1, log=True,
                    log_alpha=0.1, mod='stft'):
    # window : ('tukey',0.5) hann
    if n_downsample != 1:
        data = downsample(data, alpha=n_downsample)

    if mod == 'stft':
        zxx = scipy.signal.stft(data, window='hann', nperseg=stft_window_size, noverlap=stft_window_size - stft_stride)[
            2]
        spectrum = np.abs(zxx)
