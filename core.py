...
class Core(object):
    ...
    def epoch_save(self):
        ...
        if (self.epoch) % self.opt.network_save_freq == 0:
            os.rename(os.path.join(self.opt.save_dir, 'last.pth'),
                      os.path.join(self.opt.save_dir, self.opt.model_name + '_epoch' + str(self.epoch) + '.pth'))
            print('network saved.')
        if self.opt.gpu_id != '-1':
            self.net.cuda()
    ...
    def check_remain_time(self):
        ...
        if self.fold == 0 and self.epoch == 2:
            util.writelog('-> pre epoch cost time : ' + str(round(time.time() - self.start_time, 2)) + 's', self.opt,
                          True, True)
    ...
    def load_pool_init(self, signals, labels, sequences):
        ...
        for i in range(self.load_thread):
            if i != self.load_thread - 1:
                self.start_process(signals, labels, sequences[int(i * process_batch_num) * self.opt.batchsize:int(
                    (i + 1) * process_batch_num) * self.opt.batchsize], self.queue)
            else:
    ...
    def add_class_acc_to_tensorboard(self, tag):
        self.opt.TBGlobalWriter.add_scalars('fold' + str(self.fold + 1) + '/F1',
                                            {tag: statistics.report(self.confusion_mat)[2]}, self.step)
        self.opt.TBGlobalWriter.add_scalars('fold' + str(self.fold + 1) + '/Top1.err',
                                            {tag: statistics.report(self.confusion_mat)[3]}, self.step)
    ...
    def eval(self, signals, labels, sequences):
        ...
        if self.opt.mode == 'autoencoder':
            ...
            print('epoch:' + str(self.epoch + 1),
                  ' macro-prec,reca,F1,err,kappa: ' + str(statistics.report(self.confusion_mat)))
        ...
        self.opt.TBGlobalWriter.add_scalars('fold' + str(self.fold + 1) + '/loss',
    ...
    def train(self, signals, labels, sequences):
        ...
        for i in range(self.epoch_iter_length):
            ...
            signal, label = transforms.ToTensor(signal, label, gpu_id=self.opt.gpu_id)
            output, loss = self.forward(signal, label)
            self.opt.TBGlobalWriter.add_scalars('fold' + str(self.fold + 1) + '/loss', {'train_loss': loss.item()},
                                                self.step)
            loss.backward()
            self.optimizer.step()
        self.load_poll_terminate()
    ...
    def domain_random_loader_init(self, signals, labels, sequences, queue):
        ...
        for i in range(self.load_thread):
            if i != self.load_thread - 1:
                self.start_process(signals, labels,
                                   np.random.choice(sequences, pre_thread_iter * self.opt.batchsize, replace=True),
                                   queue)
            else:
                self.start_process(signals, labels,
                                   np.random.choice(sequences, last_thread_iter * self.opt.batchsize, replace=True),
                                   queue)
